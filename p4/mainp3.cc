...
int main(int argc, char *argv[])
{
    ...
    cv::Mat upperHalf = image(cv::Rect(0, 0, image.cols, halfRows));
    cv::Mat lowerHalf = image(cv::Rect(0, halfRows, image.cols, halfRows));

    if (rank == 0) {
        std::cout << " Hello from process " << rank
                  << " out of numprocs on " << processor_name
                  << " processor " << sched_getcpu() << std::endl;

        // remove chrominance noise
        denoise(upperHalf, upperHalf, 5);
        // apply gamma correction (move from linear output to non linear)
        gammaCorrection(upperHalf, upperHalf, 1.0, 0.0, 2.2);
        // apply color balance correction
        colorBalance(upperHalf, upperHalf, 2);
        // equalize luminance values and increase saturation
        equalization(upperHalf, upperHalf, 0.0, 1.0, 0.5);
        cv::Mat enhanced, bloomed;
        // enhance high frequency details
        enhanceDetails(upperHalf, enhanced, 20, 1.25);
        // compute bloom mask
        bloom(upperHalf, bloomed, 70, 0.9);
        // combine enhanced details with bloom mask
        screenMerge(enhanced, bloomed, upperHalf);
        // convert to 8 bit image
        upperHalf.convertTo(upperHalf, CV_8U, 1.0/255.0);

        std::cout << "upperHalf Rows: " << upperHalf.rows << " Cols: " << upperHalf.cols << std::endl;
        cv::Mat buffer = cv::Mat::eye(upperHalf.rows, upperHalf.cols, CV_8U);
        MPI_Recv(buffer.data, upperHalf.rows * upperHalf.cols, MPI_UNSIGNED_CHAR, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        //MPI_Recv(buffer.data, upperHalf.rows * upperHalf.cols, MPI_UNSIGNED_CHAR, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        cv::imwrite(outputFile, buffer);
    }
    ...
    if (rank == 1) {
        ...
        // remove chrominance noise
        denoise(lowerHalf, lowerHalf, 5);
        // apply gamma correction (move from linear output to non linear)
        gammaCorrection(lowerHalf, lowerHalf, 1.0, 0.0, 2.2);
        // apply color balance correction
        colorBalance(lowerHalf, lowerHalf, 2);
        // equalize luminance values and increase saturation
        equalization(lowerHalf, lowerHalf, 0.0, 1.0, 0.5);
        cv::Mat enhanced, bloomed;
        // enhance high frequency details
        enhanceDetails(lowerHalf, enhanced, 20, 1.25);
        // compute bloom mask
        bloom(lowerHalf, bloomed, 70, 0.9);
        // combine enhanced details with bloom mask
        screenMerge(enhanced, bloomed, lowerHalf);
        // convert to 8 bit image
        lowerHalf.convertTo(lowerHalf, CV_8U, 1.0/255.0);

        std::cout << "lowerHalf Rows: " << lowerHalf.rows << " Cols: " << lowerHalf.cols << std::endl;
	
        MPI_Send(lowerHalf.data, lowerHalf.rows * lowerHalf.cols, MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD);
        //MPI_Send(lowerHalf.data, lowerHalf.rows * lowerHalf.cols, MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD);
    }
